...
#define LevelSensorPin 23 // El sensor está conectado al GPIO33

void setup() {
  Serial.begin(115200); // Inicia la comunicación serial a 115200 bps
  pinMode(LevelSensorPin, INPUT); // Configura el pin del sensor de nivel como entrada
...
void loop() {
  ...
  int LevelSensor = analogRead(LevelSensorPin); // Lee la entrada en el pin analógico 33
  Serial.print("Nivel: ");
  Serial.println(LevelSensor); // Imprime el valor al monitor serial

  // Crear el payload con el valor del sensor
  char payload[10];
  itoa(LevelSensor, payload, 10);
